...
| 메서드 | 규칙 |
|---|---|
| `create()` | MEETING_NOT_FOUND / 생성자 비참여자 → `NOT_MEETING_PARTICIPANT` / participantUserIds에 비참여자 포함 → `NOT_MEETING_PARTICIPANT` / 생성 후 meeting.status == PLACE_VOTING이면 → SETTLING 전이 |
